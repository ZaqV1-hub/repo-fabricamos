	...
	public function webp_section_description( $cache_webp_field ) {
		...
		if ( $serving ) {
			...
			// Translators: %1$s = plugin name(s), %2$s = opening <a> tag, %3$s = closing </a> tag.
				esc_html( _n( 'You are using %1$s to serve WebP images so you do not need to enable this option. %4$s If you prefer to have AccelerateWP serve WebP for you instead, please disable WebP display in %1$s.', 'You are using %1$s to serve WebP images so you do not need to enable this option. %4$s If you prefer to have AccelerateWP serve WebP for you instead, please disable WebP display in %1$s.', count( $serving ), 'rocket' ) ), // CL. %2$sMore info%3$s.
		...
		if ( $serving_not_compatible ) {
			if ( ! $this->options_data->get( 'cache_webp', 0 ) ) {
				...
				$cache_webp_field['description'] = sprintf(
				// Translators: %1$s = plugin name(s), %2$s = opening <a> tag, %3$s = closing </a> tag.
					esc_html( _n( 'You are using %1$s to convert images to WebP. If you want AccelerateWP to serve them for you, activate this option.', 'You are using %1$s to convert images to WebP. If you want AccelerateWP to serve them for you, activate this option.', count( $serving_not_compatible ), 'rocket' ) ), // CL. %2$sMore info%3$s.
			...
			$cache_webp_field['description'] = sprintf(
			// Translators: %1$s = plugin name(s), %2$s = opening <a> tag, %3$s = closing </a> tag.
				esc_html( _n( 'You are using %1$s to convert images to WebP. AccelerateWP will create separate cache files to serve your WebP images.', 'You are using %1$s to convert images to WebP. AccelerateWP will create separate cache files to serve your WebP images.', count( $serving_not_compatible ), 'rocket' ) ), // CL. %2$sMore info%3$s.
				esc_html( wp_sprintf_l( '%l', $serving_not_compatible ) ),
				'<a href="' . esc_url( $webp_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $webp_beacon['id'] ) . '" target="_blank" rel="noopener noreferrer">',
				'</a>'
			);
		...
		if ( $creating ) {
			if ( ! $this->options_data->get( 'cache_webp', 0 ) ) {
				...
				$cache_webp_field['description'] = sprintf(
				// Translators: %1$s = plugin name(s), %2$s = opening <a> tag, %3$s = closing </a> tag.
					esc_html( _n( 'You are using %1$s to convert images to WebP. If you want AccelerateWP to serve them for you, activate this option.', 'You are using %1$s to convert images to WebP. If you want AccelerateWP to serve them for you, activate this option.', count( $creating ), 'rocket' ) ), // CL. %2$sMore info%3$s.
			...
			$cache_webp_field['description'] = sprintf(
			// Translators: %1$s = plugin name(s), %2$s = opening <a> tag, %3$s = closing </a> tag.
				esc_html( _n( 'You are using %1$s to convert images to WebP. AccelerateWP will create separate cache files to serve your WebP images.', 'You are using %1$s to convert images to WebP. AccelerateWP will create separate cache files to serve your WebP images.', count( $creating ), 'rocket' ) ), // CL. %2$sMore info%3$s.
				esc_html( wp_sprintf_l( '%l', $creating ) ),
				'<a href="' . esc_url( $webp_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $webp_beacon['id'] ) . '" target="_blank" rel="noopener noreferrer">',
				'</a>'
			);
		...
		if ( ! $this->options_data->get( 'cache_webp', 0 ) ) {
			...
			$imagify_link = 'https://docs.cloudlinux.com/user-docs/user-docs-shared-cloudlinux/#image-optimization'; // CL.

			$cache_webp_field['description'] = sprintf(
			// Translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
				esc_html__( '%5$sWe have not detected any compatible WebP plugin!%6$s%4$s If you don’t already have WebP images on your site consider using %3$sImage Optimization%2$s. %4$s If you are not using WebP do not enable this option.', 'rocket' ), // CL.
				'<a href="https://docs.cloudlinux.com/user-docs/user-docs-shared-pro-cloudlinux/#acceleratewp" target="_blank" rel="noopener noreferrer">', // CL.
				'</a>',
				$imagify_link,
				'<br>',
				'<strong>',
