...
class Deactivation {
	const DEACTIVATION_ENDPOINT = 'https://cloudlinux.com/api/clsop/deactivate-licence.php';
	...
	/**
	 * Maybe problem with .htaccess file.
	 *
	 * @note CL.
	 * @return array
	 */
	public static function maybe_problem_with_htaccess() {
		global $is_apache;
		$causes = [];

		// .htaccess problem.
		// CL.
		if ( ( clsop_is_apache2nginx() || $is_apache ) && ! rocket_direct_filesystem()->is_writable( get_home_path() . '.htaccess' ) ) {
			$causes[] = 'htaccess';
		}
		 ...
		 */
		return (array) apply_filters( 'rocket_prevent_deactivation', $causes );
	}
	...
	public static function deactivate_plugin() {
		...
		if ( ! isset( $_GET['rocket_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_GET['rocket_nonce'] ), 'force_deactivation' ) ) {
			$causes = self::maybe_problem_with_htaccess();
			// CL. Code shifted to self::maybe_problem_with_htaccess() to be able to test.
			if ( count( $causes ) ) {
