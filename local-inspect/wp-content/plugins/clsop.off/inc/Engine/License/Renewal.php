	...
	public function add_license_expire_warning( $args ): array {
		...
		if (
			...
			$message .= sprintf(
				...
				__( 'You need a valid license to continue using this feature.', /* %1$sRenew now%2$s before losing access.'*/ 'rocket' ), // CL.
			...
			$message .= sprintf(
				// translators: %1$s = <a>, %2$s = </a>.
				__( 'You need an active license to enable this option.', /* %1$sRenew now%2$s.'*/ 'rocket' ), // CL.
				'<a href="' . esc_url( $this->user->get_renewal_url() ) . '" target="_blank">',
				'</a>'
			);
		} elseif (
			...
			$doc    = 'https://docs.cloudlinux.com/user-docs/user-docs-shared-pro-cloudlinux/#acceleratewp';
			...
			$message .= sprintf(
				// translators: %1$s = <a>, %2$s = </a>.
				__( 'You need an active license to enable this option.', /* %1$sMore info%2$s.', */ 'rocket' ), // CL.
