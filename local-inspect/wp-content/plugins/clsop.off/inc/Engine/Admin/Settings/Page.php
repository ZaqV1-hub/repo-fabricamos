		...
		$this->settings->add_settings_fields(
			[
				...
				'exclude_css' => [
					...
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					sprintf( __( '<strong>3rd Party:</strong> Use either the full URL path or only the domain name, to exclude external CSS.'/* %1$sMore info%2$s*/, 'rocket' ), '<a href="' . esc_url( $exclude_css_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $exclude_css_beacon['id'] ) . '" rel="noopener noreferrer" target="_blank">', '</a>' ), // CL.
					'container_class'   => [
						'wpr-field--children',
					],
			...
			$this->settings->add_settings_fields(
				[
					'optimize_css_delivery' => [
						...
						'description'       => esc_html__( 'Optimize CSS delivery eliminates render-blocking CSS on your website.', 'rocket' ) . '<br>' . self::get_disable_feature_text( 'cpcss' ), // CL.
						'section'           => 'css',
						'page'              => 'file_optimization',
						'default'           => 0,
						'sanitize_callback' => 'sanitize_checkbox',
						'input_attr'        => [
							'disabled' => 1, // $disable_ocd ? 1 : 0, // CL.
						],
		...
		$this->settings->add_settings_sections(
			[
				...
				'dimensions_section'        => [
					...
					'type'        => 'fields_container',
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description' => sprintf( __( 'Add missing width and height attributes to images. Helps prevent layout shifts and improve the reading experience for your visitors.'/* %1$sMore info%2$s*/, 'rocket' ), '<a href="' . esc_url( $dimensions['url'] ) . '" data-beacon-article="' . esc_attr( $dimensions['id'] ) . '" target="_blank" rel="noopener noreferrer">', '</a>' ), // CL.
					'help'        => $dimensions,
					'page'        => 'media',
				],
		...
		$this->settings->add_settings_fields(
			[
				...
				'exclude_lazyload'       => [
					...
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description'     => sprintf( __( 'Specify keywords (e.g. image filename, CSS filename, CSS class, domain) from the image or iframe code to be excluded (one per line).'/* %1$sMore info%2$s*/, 'rocket' ), '<a href="' . esc_url( $exclude_lazyload['url'] ) . '" data-beacon-article="' . esc_attr( $exclude_lazyload['id'] ) . '" target="_blank" rel="noopener noreferrer">', '</a>' ), // CL.
					'section'         => 'lazyload_section',
					'page'            => 'media',
					'default'         => [],
				...
				'auto_preload_fonts'     => [
					...
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description'       => sprintf( __( 'Preload above-the-fold fonts to enhance layout stability and optimize text-based LCP elements.'/* %1$sMore info%2$s'*/, 'rocket' ), '<a href="' . esc_url( $fonts_preload['url'] ) . '" data-beacon-article="' . esc_attr( $fonts_preload['id'] ) . '" target="_blank" rel="noopener noreferrer">', '</a>' ), // CL.
				...
				'host_fonts_locally'     => [
					...
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description'       => sprintf( __( 'Download and serve fonts directly from your server. Reduces connections to external servers and minimizes font shifts.'/* %1$sMore info%2$s'*/, 'rocket' ), '<a href="' . esc_url( $fonts['url'] ) . '" data-beacon-article="' . esc_attr( $fonts['id'] ) . '" target="_blank" rel="noopener noreferrer">', '</a>' ), // CL.
					'section'           => 'font_optimization_section',
					'page'              => 'media',
					'default'           => 0,
					'sanitize_callback' => 'sanitize_checkbox',
		...
		$this->settings->add_settings_sections(
			[
				...
				'preload_links_section' => [
					...
					'type'        => 'fields_container',
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description' => sprintf( __( 'Link preloading improves the perceived load time by downloading a page when a user hovers over the link.'/* %1$sMore info%2$s*/, 'rocket' ), '<a href="' . esc_url( $preload_links['url'] ) . '" data-beacon-article="' . esc_attr( $preload_links['id'] ) . '" target="_blank">', '</a>' ), // CL.
					'help'        => [
						'id'  => $preload_links['id'],
		...
		$this->settings->add_settings_fields(
			[
				...
				'preload_excluded_uri' => [
					...
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description'       => sprintf( __( 'Specify URLs to be excluded from the preload feature (one per line).',/*%1$sMore info%2$s'*/ 'rocket' ), '<a href="' . esc_url( $exclusions['url'] ) . '" data-beacon-article="' . esc_attr( $exclusions['id'] ) . '" target="_blank">', '</a>' ), // CL.
					'placeholder'       => '/author/(.*)',
		...
		$this->settings->add_settings_sections(
			[
				...
				'cache_query_strings_section'  => [
					...
					'type'        => 'fields_container',
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description' => sprintf( __( 'Cache for query strings enables you to force caching for specific GET parameters.', 'rocket' ), '<a href="' . esc_url( $cache_query_strings_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $cache_query_strings_beacon['id'] ) . '" target="_blank">', '</a>' ), // CL.
		...
		$this->settings->add_settings_fields(
			[
				'purge_cron_interval'  => [
					...
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					// CL. 'description'       => sprintf( __( 'Reduce lifespan to 10 hours or less if you notice issues that seem to appear periodically. %1$sWhy?%2$s', 'rocket' ), '<a href="' . esc_url( $nonce_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $nonce_beacon['id'] ) . '" target="_blank">', '</a>' ),.
		...
		$this->settings->add_settings_fields(
			[
				'cache_logged_user' => [
					...
					// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
					'description'       => sprintf( __( 'User cache is great when you have user-specific or restricted content on your website.'/*<br>%1$sLearn more%2$s'*/, 'rocket' ), '<a href="' . esc_url( $user_cache_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $user_cache_beacon['id'] ) . '" target="_blank">', '</a>' ), // CL.
					'section'           => 'one_click',
					'page'              => 'addons',
				...
				apply_filters(
					...
					[
						'varnish_auto_purge' => [
							...
							// translators: %1$s = opening <a> tag, %2$s = closing </a> tag.
							'description'       => sprintf( __( 'Varnish cache will be purged each time AccelerateWP clears its cache to ensure content is always up-to-date.'/*<br>%1$sLearn more%2$s*/, 'rocket' ), '<a href="' . esc_url( $varnish_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $varnish_beacon['id'] ) . '" target="_blank">', '</a>' ), // CL.
							'section'           => 'one_click',
							'page'              => 'addons',
						...
						apply_filters(
							...
							[
								...
								// translators: %1$s and %3$s = opening <a> tag, %2$s = closing </a> tag.
									...
									$imagify_link,
									'</a>',
									// CL. '<a href="' . esc_url( $webp_beacon['url'] ) . '" data-beacon-article="' . esc_attr( $webp_beacon['id'] ) . '" target="_blank" rel="noopener noreferrer">'.
									'<a href="' . esc_url( $imagify_link ) . '" target="_blank" rel="noopener noreferrer">'
								),
								'section'           => 'one_click',
								'page'              => 'addons',
								'settings_page'     => 'webp',
		...
		rocket_notice_html(
			[
				...
				'message'        => sprintf(
						...
						__( '%1$sAccelerateWP:%2$s the plugin has been updated to the 3.19 version. New feature: %3$sPreconnect to external domains%4$s. Check out our documentation to learn more about it.', 'rocket' ),
						'<strong>',
						'</strong>',
