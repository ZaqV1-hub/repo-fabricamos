...
function rocket_warning_htaccess_permissions() {
	...
	if ( ! current_user_can( 'rocket_manage_options' )
		...
		|| ( ! clsop_is_apache2nginx() && ! $is_apache ) // CL.
	...
	$message .= '<br>' . sprintf(
		...
		'<a href="' . __( 'https://docs.cloudlinux.com/user-docs/user-docs-shared-pro-cloudlinux/#acceleratewp', 'rocket' ) . '" target="_blank">',
		'</a>'
	);
	*/
...
function rocket_thank_you_license() {
	if ( '1' === get_rocket_option( 'license' ) ) {
		...
		$message = sprintf(
			...
			'<strong>' . WP_ROCKET_PLUGIN_NAME . '</strong>',
			'<a href="https://docs.cloudlinux.com/user-docs/user-docs-shared-pro-cloudlinux/#acceleratewp" target="_blank">',
...
function rocket_notice_html( $args ) {
	...
	switch ( $args['action'] ) {
		case 'clear_cache':
			$args['action'] = '<a class="wp-core-ui button" href="' . wp_nonce_url( admin_url( 'admin-post.php?action=purge_cache&type=all' . \WP_Rocket\Buffer\Cache::get_referrer() ), 'purge_cache_all' ) . '">' . __( 'Clear cache', 'rocket' ) . '</a>'; // CL.
			break;
		case 'clear_used_css':
			$params = [
		...
		case 'force_deactivation':
			...
			break;
		case 'clsop_notice_link':
			// CL.
			if ( ! empty( $args['action_link_text'] ) ) {
				$args['action'] = '<a class="wp-core-ui button" style="margin-right:10px" href="' . wp_nonce_url( admin_url( $args['action_link_url'] ), $args['action_nonce'] ) . '">' . $args['action_link_text'] . '</a>';
			}
			break;
	}
 ...
 * @return array
 */
function clsop_get_notices() {
	$file    = rocket_get_constant( 'WP_ROCKET_UPDATE_PATH' ) . DIRECTORY_SEPARATOR . 'notices.json';
	$notices = [];

	if ( @is_readable( $file ) ) { // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
		$notices_content = file_get_contents( $file ); //phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents

		if ( is_string( $notices_content ) ) {
			$notices = json_decode( $notices_content, true );
		}
	}
 ...
 * @since 3.10.4
 */
function clsop_notice() {
	if ( isset( $_GET['action'] ) && 'upgrade-plugin' === $_GET['action'] ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
		return;
	}
	if ( current_user_can( 'rocket_manage_options' ) ) {
		$boxes   = get_user_meta( get_current_user_id(), 'rocket_boxes', true );
		$notices = clsop_get_notices();
		$version = rocket_get_constant( 'WP_ROCKET_VERSION' );

		// Ignoring all notices with versions lower than the plugin version.
		foreach ( $notices as $key => $notice ) {
			if (
				empty( $notice['maximum_display_version'] ) ||
				in_array( 'clsop_notice_' . $notice['maximum_display_version'], (array) $boxes, true ) ||
				version_compare( $version, $notice['maximum_display_version'], '>=' )
			) {
				unset( $notices[ $key ] );
			}
		}
		...
		$notice = array_reduce(
			...
			function ( $first_notice, $second_notice ) {
				...
				if ( version_compare( $first_notice['maximum_display_version'], $second_notice['maximum_display_version'], '>' ) ) {
					return $first_notice;
				} else {
					return $second_notice;
				}
		...
		$notice_box              = 'clsop_notice_' . $notice['maximum_display_version'];
		$notice_status           = $notice['type'] ?? 'error';
		$notice_text             = $notice['text'] ?? '';
		$notice_action_link_url  = $notice['action_link_url'] ?? '';
		$notice_action_nonce     = $notice['action_nonce'] ?? '';
		$notice_action_link_text = $notice['action_link_text'] ?? '';

		// Clear plugins update cache - no more than once per hour.
		$cl_update_plugins_debounce = get_transient( 'cl_update_plugins_debounce' );

		if ( empty( $cl_update_plugins_debounce ) ) {
			wp_clean_plugins_cache();
			set_transient( 'cl_update_plugins_debounce', '1', HOUR_IN_SECONDS );
		}
		...
		rocket_notice_html(
			[
				...
				'dismissible'      => '',
				'message'          => $notice_text,
				'dismiss_button'   => $notice_box,
				'action'           => 'clsop_notice_link',
				'action_link_url'  => $notice_action_link_url,
				'action_nonce'     => $notice_action_nonce,
				'action_link_text' => $notice_action_link_text,
			]
		);
	}
}
add_action( 'admin_notices', 'clsop_notice' );
