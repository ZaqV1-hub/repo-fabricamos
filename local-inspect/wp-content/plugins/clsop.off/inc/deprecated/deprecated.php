...
if ( ! function_exists( 'rocket_white_label' ) ) {
	...
	function rocket_white_label( $plugins ) {
		...
		$plugins['clsop/clsop.php'] = array(
			'Name'        => get_rocket_option( 'wl_plugin_name' ),
			'PluginURI'   => get_rocket_option( 'wl_plugin_URI' ),
			'Version'     => isset( $plugins['clsop/clsop.php']['Version'] ) ? $plugins['clsop/clsop.php']['Version'] : '',
			'Description' => reset( ( $white_label_description ) ),
			'Author'      => get_rocket_option( 'wl_author' ),
			'AuthorURI'   => get_rocket_option( 'wl_author_URI' ),
			'TextDomain'  => isset( $plugins['clsop/clsop.php']['TextDomain'] ) ? $plugins['clsop/clsop.php']['TextDomain'] : '',
			'DomainPath'  => isset( $plugins['clsop/clsop.php']['DomainPath'] ) ? $plugins['clsop/clsop.php']['DomainPath'] : '',
		);
...
if ( ! function_exists( 'rocket_php_warning' ) ) {
	...
	function rocket_php_warning() {
		...
		echo '<div class="notice notice-error"><p>' . sprintf( __( '%1$s %2$s requires at least PHP %3$s to function properly. To use this version, please ask your web host how to upgrade your server to PHP %3$s or higher.'/* If you are not able to upgrade, you can rollback to the previous version by using the button below.*/, 'rocket' ), WP_ROCKET_PLUGIN_NAME, WP_ROCKET_VERSION, '5.3' ) . '</p>' // CL.
		/*
		CL.
