...
if ( ! function_exists( 'get_rocket_footprint' ) ) :
	...
	function get_rocket_footprint( $debug = true ) {
		...
		$footprint = defined( 'WP_ROCKET_WHITE_LABEL_FOOTPRINT' ) ?
						...
						"\n" . '<!-- Performance optimized by ' . WP_ROCKET_PLUGIN_NAME .'.' /*. '. Learn more: https://cloudlinux.com'*/; // CL.
		if ( $debug ) {
...
if ( ! function_exists( 'rocket_user_agent' ) ) :
	...
	function rocket_user_agent( $user_agent ) {
		...
		$new_ua      = sprintf( '%s;AccelerateWP|%s%s|%s|%s|%s|%s;', $user_agent, WP_ROCKET_VERSION, $bonus, $consumer_key, $consumer_email, esc_url( home_url() ), $php_version );
...
if ( ! function_exists( 'rocket_force_info' ) ) :
	...
	function rocket_force_info( $bool, $action, $args ) {
		...
		if ( 'plugin_information' === $action && 'clsop' === $args->slug ) {
			return new stdClass();
		}
		return $bool;
	}
...
if ( ! function_exists( 'rocket_force_info_result' ) ) :
	...
	function rocket_force_info_result( $res, $action, $args ) {
		...
		if ( 'plugin_information' === $action && isset( $args->slug ) && 'clsop' === $args->slug && isset( $res->external ) && $res->external ) {
			...
			if ( is_wp_error( $request ) ) {
				...
				$res = new WP_Error( 'plugins_api_failed', sprintf( __( 'An unexpected error occurred. Something may be wrong with external or this server&#8217;s configuration.'/* If you continue to have problems, <a href="%s">contact support</a>.*/, 'rocket' ), rocket_get_external_url( 'support', array( // CL.
