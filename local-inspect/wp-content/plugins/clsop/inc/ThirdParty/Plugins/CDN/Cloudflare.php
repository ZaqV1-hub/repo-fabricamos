...
class Cloudflare implements Subscriber_Interface, DeactivationInterface {
	...
	public function update_addon_field( $settings ) {
		...
		$settings['do_cloudflare']['description']   = __( 'Cloudflare cache will be purged each time AccelerateWP clears its cache to ensure content is always up-to-date.', 'rocket' );
	...
	public function display_apo_cookies_notice() {
		...
		$message .= sprintf(
			...
			__( 'You should either disable Cloudflare APO or check with the theme/plugin requiring the use of “Dynamic Cookies Cache” developers for an alternative way to be page-cache friendly.', /* %1$sMore info%2$s',*/ 'rocket' ), // CL.
			...
			rocket_notice_html(
				[
					...
					'message'     => sprintf(
						...
						__( '%1$sAccelerateWP:%2$s You are using "Separate cache files for mobile devices". You need to activate "Cache by Device Type" %3$ssetting%5$s on Cloudflare APO to serve the right version of the cache.', 'rocket' ), // CL.
						'<strong>',
						'</strong>',
