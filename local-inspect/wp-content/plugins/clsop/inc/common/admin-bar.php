...
function rocket_admin_bar( $wp_admin_bar ) {
	...
	if ( ! empty( $_SERVER['REQUEST_URI'] ) ) {
		$uri = filter_var( wp_unslash( $_SERVER['REQUEST_URI'] ), FILTER_SANITIZE_URL ); // CL.
		/**
	...
	if ( current_user_can( 'rocket_manage_options' ) ) {
		...
		if ( isset( $rocketcdn_status['subscription_status'] ) && 'running' === $rocketcdn_status['subscription_status'] ) { // CL.
			$wp_admin_bar->add_menu(
				[
					'parent' => 'wp-rocket',
					'id'     => 'purge-cdn-cache',
