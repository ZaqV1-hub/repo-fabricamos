...
function flush_rocket_htaccess( $remove_rules = false ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
	...
	// CL. Don't add GZIP rules when LiteSpeed.
	if ( clsop_is_litespeed() ) {
	 ...
	 */
	if ( ( ! clsop_is_apache2nginx() && ! $is_apache ) || ( apply_filters( 'rocket_disable_htaccess', false ) && ! $remove_rules ) ) {
		return false;
	}
	...
	/**
	 * CL.
	 * Force .htaccess file creation for apache2nginx with WordPress Rewrite rules.
	 */
	if ( clsop_is_apache2nginx() && ! $remove_rules ) {
	...
	$start_marker = '# BEGIN AccelerateWP';
	$end_marker   = '# END AccelerateWP';
	...
	while ( ! feof( $pointer ) ) {
		// CL. Replace old naming CloudLinux Site Optimization with AccelerateWP.
		$line = rtrim( fgets( $pointer ), "\r\n" );
		if ( false !== strpos( $line, 'CloudLinux Site Optimization' ) ) {
			$line = str_replace( 'CloudLinux Site Optimization', 'AccelerateWP', $line );
		}
		$lines[] = $line;
	}
...
function get_rocket_htaccess_marker() { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
	...
	*/
	...
	if ( \WP_Rocket\Buffer\Cache::can_generate_caching_files() && ( clsop_is_apache2nginx() || ! is_rocket_generate_caching_mobile_files() ) ) {
...
function get_rocket_htaccess_mod_rewrite() { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
	...
	// CL. added conditions for nginx and mobile
	$is_cache_mobile = is_rocket_cache_mobile();
	if ( clsop_is_apache2nginx() && $is_cache_mobile ) {
		$rules .= 'RewriteRule .* - [E=CACHEABLE:true]' . PHP_EOL;

		$rules .= 'RewriteCond %{HTTP:SEC-CH-UA-MOBILE} =?1 [NC,OR]' . PHP_EOL;
		$rules .= 'RewriteCond %{HTTP_USER_AGENT} android.+mobile|blackberry|iemobile|ip(hone|od)|kindle|opera\ m(ob|in)i|palm(\ os)?|samsungbrowser|huawei|miui|redmi|vivo|oneplus|oppo|realme|nokia|motorola|chrome.+mobile|crios|edg|fxios|mobile.+safari|Mobile|Silk/|Opera\ Mini|Opera\ Mobi [NC,OR]' . PHP_EOL;
		$rules .= 'RewriteCond %{HTTP:X-Wap-Profile} !="" [OR]' . PHP_EOL;
		$rules .= 'RewriteCond %{HTTP:Profile} !=""' . PHP_EOL;
		$rules .= 'RewriteRule .* - [E=MOBILE:true]' . PHP_EOL;

		$rules .= 'RewriteCond %{ENV:CACHEABLE} true' . PHP_EOL;
		$rules .= 'RewriteCond %{ENV:MOBILE} true' . PHP_EOL;
		$rules .= 'RewriteCond "' . $cache_dir_path . '/.mobile-active" -f' . PHP_EOL;
		$rules .= 'RewriteCond "' . $cache_dir_path . '/index-mobile%{ENV:WPR_SSL}%{ENV:WPR_WEBP}.html' . $enc . '" -f' . PHP_EOL;
		$rules .= 'RewriteRule .* "' . $cache_root . $http_host . '%{REQUEST_URI}/index-mobile%{ENV:WPR_SSL}%{ENV:WPR_WEBP}.html' . $enc . '" [L]' . PHP_EOL;
...
function get_rocket_htaccess_files_match() { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals
	...
	$rules .= '<IfModule mod_headers.c>' . PHP_EOL;
	$rules .= 'Header set X-Powered-By "AccelerateWP/' . WP_ROCKET_VERSION . '"' . PHP_EOL;
