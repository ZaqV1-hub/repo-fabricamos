...
function rocket_settings_action_links( $actions ) {
	...
	/*
	CL. Temporary off external links
	array_unshift( $actions, sprintf( '<a href="%s">%s</a>', 'https://docs.cloudlinux.com/user-docs/user-docs-shared-pro-cloudlinux/#acceleratewp', __( 'Support', 'rocket' ) ) );
...
function rocket_plugin_row_meta( $plugin_meta, $plugin_file ) {
	if ( 'clsop/clsop.php' === $plugin_file ) {
...
function rocket_post_row_actions( $actions, $post ) {
	...
	$url                     = wp_nonce_url( admin_url( 'admin-post.php?action=purge_cache&type=post-' . $post->ID ), 'purge_cache_post-' . $post->ID . \WP_Rocket\Buffer\Cache::get_referrer() ); // CL.
...
function rocket_user_row_actions( $actions, $user ) {
	...
	$url                     = wp_nonce_url( admin_url( 'admin-post.php?action=purge_cache&type=user-' . $user->ID ), 'purge_cache_user-' . $user->ID . \WP_Rocket\Buffer\Cache::get_referrer() ); // CL.
	$actions['rocket_purge'] = sprintf( '<a href="%s">%s</a>', $url, __( 'Clear this cache', 'rocket' ) );
...
function rocket_send_analytics_data() {
	...
	/*
	// CL. Turn off sending analytics data
