	...
	if ( ! isset( $cf_email, $cf_api_key ) || empty( $cf_email ) || empty( $cf_api_key ) ) {
		return new WP_Error(
			...
			/* translators: %1$s = opening link; %2$s = closing link */
				__( 'Cloudflare email, API key and Zone ID are not set.'/* Read the %1$sdocumentation%2$s for further guidance.*/, 'rocket' ), // CL.
				// translators: Documentation exists in EN, FR; use localized URL if applicable.
				'<a href="' . esc_url( __( 'https://docs.cloudlinux.com/user-docs/user-docs-shared-pro-cloudlinux/#acceleratewp', 'rocket' ) ) . '" rel="noopener noreferrer" target="_blank">',
				'</a>'
			)
		);
	}
...
function get_rocket_cloudflare_api_instance() {
	...
	if ( ! isset( $cf_email, $cf_api_key ) ) {
		return new WP_Error(
			...
			sprintf(
			/* translators: %1$s = opening link; %2$s = closing link */
				__( 'Cloudflare email and API key are not set.'/* Read the %1$sdocumentation%2$s for further guidance.*/, 'rocket' ), // CL.
				// translators: Documentation exists in EN, FR; use localized URL if applicable.
				'<a href="' . esc_url( __( 'https://docs.cloudlinux.com/user-docs/user-docs-shared-pro-cloudlinux/#acceleratewp', 'rocket' ) ) . '" rel="noopener noreferrer" target="_blank">',
				'</a>'
			)
...
function rocket_tag_row_actions( $actions, $term ) {
	...
	$url                     = wp_nonce_url( admin_url( 'admin-post.php?action=purge_cache&type=term-' . $term->term_id . '&taxonomy=' . $taxnow . \WP_Rocket\Buffer\Cache::get_referrer() ), 'purge_cache_term-' . $term->term_id ); // CL.
