...
<?php
	printf(
		...
		esc_html__( 'Generate specific Critical Path CSS for this post.'/* %1$sMore info%2$s*/, 'rocket' ), // CL.
...
<?php
	printf(
		// translators: %1$s = opening link tag, %2$s = closing link tag.
		esc_html__( 'This post uses specific Critical Path CSS.'/* %1$sMore info%2$s*/, 'rocket' ), // CL.
		'<a href="' . esc_url( $data['beacon']['url'] ) . '" data-beacon-article="' . esc_attr( $data['beacon']['id'] ) . '" target="_blank" rel="noopener noreferrer">',
		'</a>'
		);
	?>
